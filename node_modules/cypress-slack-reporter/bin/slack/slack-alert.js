...
const slackRunner = async ({ ciProvider, vcsRoot, reportDir, videoDir, screenshotDir, customUrl = "", onlyFailed = false, customText = "", useOnlyCustomUrl = false, }) => {
    try {
...
const buildHTMLReportURL = async ({ reportDir, artefactUrl, useOnlyCustomUrl, }) => {
    if (!useOnlyCustomUrl) {
...
const getArtefactUrl = ({ vcsRoot, ciEnvVars, ciProvider, customUrl, }) => {
    if (customUrl) {
        return customUrl;
    }
...
const resolveCIProvider = (ciProvider) => {
